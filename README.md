 ...
 * In case the script fails to connect to the websockets, it might be due to compatibility issues with the websocket-client library around version 0.53.0. Trying upgrade to the latest version via `pip3 install --upgrade websocket-client` if running the script locally or rebuild the docker containers.
